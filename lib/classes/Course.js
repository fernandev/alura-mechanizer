...
class Course {
	...
	get folder() {
		return Utility.findObjectInObjectList(Career, this, '_courses', '_link').folder + '/' + Utility.cleanFileName(this.name);
	}

	static addToCourseList(course) {
		Course._list = Course._list || [];
		if (!Utility.findObjectInObjectList(Course, course, '_link')) {
