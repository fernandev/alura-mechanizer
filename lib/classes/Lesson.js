...
class Lesson {
	...
	get folder() {
		return Utility.findObjectInObjectList(Course, this, '_lessons', '_link').folder + '/' + Utility.cleanFileName(this.name);

	}

	static addToLessonList(lesson) {
		Lesson._list = Lesson._list || [];
		if (!Utility.findObjectInObjectList(Lesson, lesson, '_link')) {
